...
public class CellNoise
{
	...
	public CellNoise(long seed, short i)
	{
		this.seed = seed;
		this.distanceMethod = i;
	}
...
//				double zDist = zPos - z;
				...
				if(dist < dCandidate)
				{
					...
					/*
					 * dNeighbour = dCandidate; xNeighbour = xCandidate;
	...
	public double noise(double x, double y, double z, double frequency)
	{
		...
		if(useDistance)
		{
			...
			return ((double) CellNoise.valueNoise3D((int) (Math.floor(xCandidate)), (int) (Math.floor(yCandidate)),
