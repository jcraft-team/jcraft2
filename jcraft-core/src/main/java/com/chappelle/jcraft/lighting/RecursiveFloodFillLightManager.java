...
public class RecursiveFloodFillLightManager implements LightManager
{
	private World world;
	...
	public RecursiveFloodFillLightManager(World world)
	{
		this.world = world;
...
//		initSunlight();
	...
	private void propagateSunlightDown(Chunk chunk, int x, int y, int z)
	{
		...
		for(int i = y; i >= 0; i--)
		{
			...
			if(block == null || block.isTransparent)
			{
				...
				LightAxis(chunk, x, i, z, LightType.SKY, Direction.BACK);
				LightAxis(chunk, x, i, z, LightType.SKY, Direction.FRONT);
			}
		}
	}

	private void addLight(Chunk chunk, int x, int y, int z, LightType lightType, int amount)
	{
		chunk.setLight(x, y, z, lightType, amount);
	...
	void LightAxis(Chunk chunk, int x, int y, int z, LightType lightType, Direction direction)
	{
		if(chunk == null)
		{
			return;
		}
		for(int i = chunk.getLight(x, y, z) - 1; i != 0; i--)
		{
			switch(direction)
			{
			case RIGHT:		x++; break;
			...
			else if(chunk.getLight(x, y, z) >= i)
			{
				break;
			}

			chunk.setLight(x, y, z, lightType, i);

			switch(direction)
			{
			case RIGHT:
				LightAxis(chunk, x, y, z, lightType, Direction.UP);
				LightAxis(chunk, x, y, z, lightType, Direction.DOWN);
				LightAxis(chunk, x, y, z, lightType, Direction.FRONT);
				LightAxis(chunk, x, y, z, lightType, Direction.BACK);
				break;
			case LEFT:
				LightAxis(chunk, x, y, z, lightType, Direction.UP);
				LightAxis(chunk, x, y, z, lightType, Direction.DOWN);
				...
				break;
			case UP:
				LightAxis(chunk, x, y, z, lightType, Direction.LEFT);
				LightAxis(chunk, x, y, z, lightType, Direction.RIGHT);
				LightAxis(chunk, x, y, z, lightType, Direction.FRONT);
				LightAxis(chunk, x, y, z, lightType, Direction.BACK);
				break;
			case DOWN:
	...
	private boolean needsLightUpdated(Chunk chunk, int x, int y, int z, LightType lightType, int lightLevel)
	{
		return chunk != null && isNotOpaque(chunk.getBlock(x, y, z)) && chunk.getLight(x, y, z, lightType) + 2 <= lightLevel;
	}
	...
	public void initSunlight(Chunk chunk)
	{
		...
		for(int x = 0; x < 16; x++)
		{
			for(int z = 0; z < 16; z++)
			{
				propagateSunlightDown(chunk, x, y, z);
			}
		}
	}
}