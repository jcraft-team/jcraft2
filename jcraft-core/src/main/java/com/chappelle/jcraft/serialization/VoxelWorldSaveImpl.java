 ...
 */
public class VoxelWorldSaveImpl implements VoxelWorldSave
{
	private static final Logger log = Logger.getLogger(VoxelWorldSaveImpl.class.getName());
	 ...
	 */
	private LinkedList<AbstractMap.SimpleEntry<ChunkCoordIntPair, int[][][]>> writerQueue;
	...
	public VoxelWorldSaveImpl(File worldSaveDir)
	{
		try
		{
	...
	{
//		int chunksSaved = 0;
		HashMap<ChunkCoordIntPair, int[][][]> updateJobs = new HashMap<>();
		...
		synchronized(this.writerQueueLock)
		{
			...
			{
				// Get job from queue
				AbstractMap.SimpleEntry<ChunkCoordIntPair, int[][][]> queueEntry = this.writerQueue.poll();
...
//			long startTime = System.currentTimeMillis();
			...
			for(Entry<ChunkCoordIntPair, int[][][]> entry : updateJobs.entrySet())
			{
				// Get chunk position
		...
		synchronized(this.connectionLockObject)
		{
			try
			{
				...
				if(chunkData != null)
				{
					...
					int[][][] voxelData = new int[16][256][16];
					for(int x = 0; x < 16; x++)
						for(int y = 0; y < 256; y++)
							for(int z = 0; z < 16; z++)
