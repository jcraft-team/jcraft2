...
package com.chappelle.jcraft.util.concurrency;
...
public final class TaskMaster<T extends Task>
{
	...
	{
		return new TaskMaster<T>(name, threads, new LinkedBlockingQueue<T>());
	}

	public static <T extends Task & Comparable<? super T>> TaskMaster<T> createPriorityTaskMaster(String name,
			int threads, int queueSize)
	{
		return new TaskMaster<T>(name, threads, new PriorityBlockingQueue<T>(queueSize));
	}

	public static <T extends Task> TaskMaster<T> createPriorityTaskMaster(String name, int threads, int queueSize,
			Comparator<T> comparator)
	{
		return new TaskMaster<>(name, threads, new PriorityBlockingQueue<>(queueSize, comparator));
	}

	public static <T extends Task> TaskMaster<T> createDynamicPriorityTaskMaster(String name, int threads,
			Comparator<T> comparator)
	{
	...
	/**
	 * Offers a task to this task master. This does not block, but may fail if
	 * the queue is full.
	 *
	 * @param task
	...
	/**
	 * Adds a task to this task master. This blocks until the task can be added
