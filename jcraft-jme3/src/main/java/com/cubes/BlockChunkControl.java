...
public class BlockChunkControl extends AbstractControl implements BitSerializable, Chunk
{
    ...
    public BlockChunkControl(BlockTerrainControl terrain, int x, int y, int z)
    {
    	...
    	blockState = new BlockState[terrain.getSettings().getChunkSizeX()][terrain.getSettings().getChunkSizeY()][terrain.getSettings().getChunkSizeZ()];
    	lights = new LightMap(new Vector3Int(terrain.getSettings().getChunkSizeX(), terrain.getSettings().getChunkSizeY(), terrain.getSettings().getChunkSizeZ()), location);
    }
    ...
    private void updateBlockState(Vector3Int location){
        ...
        for(int i=0;i<Block.Face.values().length;i++){
            ...
            BlockChunkControl chunk = (BlockChunkControl)terrain.getChunk(neighborLocation);//FIXME: Casting?
            if(chunk != null){
